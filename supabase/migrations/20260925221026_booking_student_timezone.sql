-- The student's own time zone, captured from their browser when they book,
-- so emails can show the lesson in their local time.
alter table public.bookings add column student_timezone text;

-- Same function plus an optional time zone. Postgres treats a new parameter
-- list as a new function, so replace the old one rather than overloading it.
drop function public.request_individual_booking(timestamptz, timestamptz);
...
begin
  ...
  insert into public.bookings (session_slot_id, student_id, status, is_admin_override, student_timezone)
  values (
    v_slot_id,
    v_uid,
...
revoke execute on function public.request_individual_booking(timestamptz, timestamptz, text) from public, anon;
grant execute on function public.request_individual_booking(timestamptz, timestamptz, text) to authenticated;
